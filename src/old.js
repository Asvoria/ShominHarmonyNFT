...
const contractAdds = '0x01025cB22BA463389cd6741C9E7a510C170c6c37'
...
const makeIPFS = async () => {
  if (await !ItemStatus1) {
    await makeSoldStamp('https://Asvoria.github.io/ShominHarmonyNFT/miniso_marvel_speaker.jpg')
    invoiceURI1 = invoiceURI
  }

  if (await !ItemStatus2) {
    await makeSoldStamp('https://Asvoria.github.io/ShominHarmonyNFT/craftholic_pouch.jpg')
...
const btnDecryptClick = async () => {
  try {
    ...
    textArea.innerText = await ethereum
      .request({ method: 'eth_decrypt', params: [document.getElementById('enc').innerText, ethereum.selectedAddress] })
