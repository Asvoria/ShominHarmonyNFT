...
const runMetamask = () => {
  ...
  const onClickConnect = async () => {
    try {
      ...
      const BalanceInContractONE = await SHOMINcontractONE.methods.balanceOf({ 'owner': toString(_accounts[0]) }).encodeABI().call()
      if (BalanceInContractONE === 0) {
        const BalanceInContractBNB = await SHOMINcontractBNB.methods.balanceOf({ 'owner': toString(_accounts[0]) }).encodeABI().call()
